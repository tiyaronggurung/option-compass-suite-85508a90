...
//
// v1.1 — Relaxed band guards:
//   • minVolume reduced (10–25). Volume below floor is a *soft* penalty when
//     open interest is healthy (>= minOI * 2). It is NOT a hard reject by itself.
//   • maxSpreadPct increased ~2pp across profiles. Extreme spreads still reject.
//
// Paper-only. Does NOT affect scoring weights, thresholds, scanner, or signal generation.
...
export type BandPrefs = {
  ...
  deltaMin: number;
  deltaMax: number;
  maxSpreadPct: number; // e.g. 7 means 7%
  minOI: number;
  minVolume: number;
};
...
export const EXTREME_SPREAD_PCT = 25;      // hard reject above this regardless of band
export const BEST_EFFORT_MIN_SCORE = 35;   // minimum score required to surface best-effort pick
