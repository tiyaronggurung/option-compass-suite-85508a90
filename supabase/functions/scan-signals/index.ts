...
function isMarketOpenET(now = new Date()): { open: boolean; reason: string } {
  ...
  if (US_MARKET_HOLIDAYS.has(ymd)) return { open: false, reason: "holiday" };
  const mins = hour * 60 + minute;
  const open = 9 * 60 + 30;
  const close = US_MARKET_EARLY_CLOSE_DAYS.has(ymd) ? 13 * 60 : 16 * 60;
    ...
    try {
      ...
      const { error } = await admin.from("signals").insert({
        ...
        score_components: institutional ? {
          ...
          fallback_count: institutional.fallback_count,
          fallback_components: institutional.fallback_components,
          fallback_penalty: institutional.fallback_penalty,
        ...
        fallback_count: institutional?.fallback_count ?? null,
