...
Deno.serve(async (req) => {
  ...
  for (const a of alerts) {
    ...
    // NOTE: Auto-close of the linked paper_trade was intentionally removed.
    // The user wants exits to be MANUAL only. We still update the alert
    // lifecycle (alert_status, hit_t*_at, stopped_at) so the UI can surface a
    // "consider closing" hint, but we never touch paper_trades.status here.
